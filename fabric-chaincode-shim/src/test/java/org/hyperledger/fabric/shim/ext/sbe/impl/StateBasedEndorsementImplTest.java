...
public class StateBasedEndorsementImplTest {
    ...
    public void addOrgs() {
        ...
        ep.addOrgs(StateBasedEndorsement.RoleType.RoleTypePeer, "Org1");

        byte[] epBytes = ep.policy();
        assertThat(epBytes, is(not(nullValue())));
        assertTrue(epBytes.length > 0);
        byte[] expectedEPBytes = StateBasedEndorsementUtils.signedByFabricEntity("Org1", MspPrincipal.MSPRole.MSPRoleType.PEER).toByteString().toByteArray();
    ...
    public void delOrgs() {
        ...
        byte[] initEPBytes = StateBasedEndorsementUtils.signedByFabricEntity("Org1", MspPrincipal.MSPRole.MSPRoleType.PEER).toByteString().toByteArray();
        ...
        ep.addOrgs(StateBasedEndorsement.RoleType.RoleTypeMember, "Org2");
        ep.delOrgs("Org1");
        ...
        assertThat(epBytes, is(not(nullValue())));
        assertTrue(epBytes.length > 0);
        byte[] expectedEPBytes = StateBasedEndorsementUtils.signedByFabricEntity("Org2", MspPrincipal.MSPRole.MSPRoleType.MEMBER).toByteString().toByteArray();
        assertArrayEquals(expectedEPBytes, epBytes);
    }
