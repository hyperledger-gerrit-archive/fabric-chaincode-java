...
public class ContractDefinitionImpl implements ContractDefinition {
	...
	public ContractDefinitionImpl(Class<?> cl) throws InstantiationException, IllegalAccessException {
	...
	public Routing getUnkownRoute() {
		...
		rd.contract = this.getContractImpl();
		try {
			rd.method = rd.contract.getClass().getMethod("unknownTransaction", new Class<?>[] {});
		...
		rd.type = TransactionType.DEFAULT;
		rd.clazz = this.getContractImpl().getClass();
		return rd;
	}
