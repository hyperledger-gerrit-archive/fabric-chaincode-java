...
public class MetadataBuilder {
	...
	public static void addComponent(DataTypeDefinition datatype) {
		...
		Map propertiesMap =  datatype.getProperties().entrySet().stream()
                .collect(Collectors.toMap(Entry::getKey, e -> (  ((PropertyDefinition)e.getValue()).getSchema()   )));
...
//			paramMap.put("schema", pd.getSchema());
	...
	private static JSONObject metadata() {
		...
		metadata.put("$schema", "https://fabric-shim.github.io/relerase-1.4/contract-schema.json");
