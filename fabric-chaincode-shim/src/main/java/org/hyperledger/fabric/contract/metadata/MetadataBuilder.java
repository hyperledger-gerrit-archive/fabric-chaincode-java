...
public class MetadataBuilder {
	...
	// Specific Map that helps with the case where if there's no value then do not insert
	// the 
	...
	public static void validate() {
		...
		try (InputStream inputStream = MetadataBuilder.class.getClassLoader().getResourceAsStream("contract-schema.json")) {
	...
	// TODO support the primative types
	// TODO support the complex objects
	// TODO support the primitive types
	...
	// 	if (args.length % 2 != 0) {
	// 		throw new RuntimeException("Mismatched number of key-value elements");
	// 	}
	// 	if (args.length == 0) {
	// 		return Collections.emptyMap();
	// 	}

	// 	Object[][] data = new Object[args.length / 2][2];
	// 	for (int i = 0; i < args.length; i++) {
	// 		data[i / 2][i % 2] = args[i];
	// 	}
	// 	Map<String, Object> map = Stream.of(data).collect(Collectors.toMap(p -> (String) p[0], p -> (Object) p[1]));
	// 	return map;
	// }
}
