...
public class ChaincodeInnvocationTask implements Callable<ChaincodeMessage> {
	...
	public ChaincodeMessage call() {
		...
		try {
			...
			// result is what will be sent to the peer as a response to this innvocation
			...
			if (result.getStatus().getCode() >= Chaincode.Response.Status.INTERNAL_SERVER_ERROR.getCode()) {
				...
				logger.severe(() -> String.format("[%-8.8s] Invoke failed. Sending %s", message.getTxid(), ERROR));
				finalResponseMessage = ChaincodeMessageFactory.newErrorEventMessage(message.getChannelId(), message.getTxid(),
						result.getMessage(), stub.getEvent());
			} else {
				...
				finalResponseMessage = ChaincodeMessageFactory.newCompletedEventMessage(message.getChannelId(), message.getTxid(),
						result, stub.getEvent());
			}
	...
	/**
	 * Use the Key as determinator of equality
	 * 
	 * @param task
	 * @return
	 */
	...
	/**
	 * Send the chaincode message back to the peer and wait for the response to come
	 * back
	 */
	private ByteString invokeChaincodeSupport(final ChaincodeMessage message) {

		// send the message
		logger.info(()->"Sending message to the peer");
