...
public abstract class ChaincodeBase implements Chaincode {
	...
	private static final Logger logger = Logger.getLogger(ChaincodeBase.class);
	...
	private static final boolean oldSchool = Boolean.parseBoolean(System.getenv("OLDSCHOOL"));
	...
	public void start(String[] args) {
		try {
			...
			logger.error("Chaincode could not start" + e);
		}
	}
	...
	protected void connectToPeer() throws IOException {
		...
		logger.info("Running with algorthms " + (oldSchool ? "old school" : "new school"));
		System.out.println("School=" + oldSchool);
		if (oldSchool) {
			final Handler handler = new Handler(chaincodeId, this);
			new ChaincodeSupportStream(channelBuilder, handler::onChaincodeMessage,
					handler::nextOutboundChaincodeMessage);
		} else {
	...
	protected void initializeLogging() {
		...
		for (String name : Collections.list(logManager.getLoggerNames())) {
			...
			if (logger != null) {
				logger.setLevel(chaincodeLogLevel);
				for (java.util.logging.Handler handler : logger.getHandlers()) {
					handler.setLevel(chaincodeLogLevel);
...
////			});
//		}
		rootLogger.info("Test INFO message: " + chaincodeLogLevel);
		rootLogger.fine("Test FINE message: " + chaincodeLogLevel);
		rootLogger.warning("Test WARNING message: " + chaincodeLogLevel);
	}
	...
	protected void processCommandLineOptions(String[] args) {
		...
		try {
			...
			logger.warn(() -> "cli parsing failed with exception" + logger.formatError(e));
		}
	...
	public Properties getProperties() {
		if (this.props == null) {
			...
			try (InputStream inStream = cl.getResourceAsStream("config.props")) {
				...
				logger.warning(() -> "Can not open the properties file for input " + logger.formatError(e));
			}
