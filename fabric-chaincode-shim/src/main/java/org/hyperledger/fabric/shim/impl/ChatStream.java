...
public class ChatStream implements StreamObserver<ChaincodeShim.ChaincodeMessage> {
	...
	public synchronized void serialSend(ChaincodeShim.ChaincodeMessage message) {
		if(logger.isDebugEnabled()) {
			logger.debug(format("[%-8.8s] Sending %s message to peer.", message.getTxid(), message.getType()));
		}
		if (logger.isTraceEnabled()) {
			logger.trace(format("[%-8.8s] ChaincodeMessage: %s", message.getTxid(), toJsonString(message)));
		}
		try {
			this.streamObserver.onNext(message);
			if (logger.isTraceEnabled()) {
				logger.trace(format("[%-8.8s] %s message sent.", message.getTxid(), message.getType()));
			}
		} catch (Exception e) {
			logger.error(String.format("[%-8.8s] Error sending %s: %s", message.getTxid(), message.getType(), e));
	...
	public void onNext(ChaincodeShim.ChaincodeMessage message) {
		...
		try {
			if(logger.isDebugEnabled()) {
				logger.debug(String.format("[%-8.8s] Received message %s from org.hyperledger.fabric.shim", message.getTxid(), message.getType()));
			}
			handler.handleMessage(message);
		} catch (Exception e) {
			logger.error(String.format("[%-8.8s] Error handling message %s: %s", message.getTxid(), message.getType(), e));
			System.exit(-1);
		}
	}
	...
	public void receive() throws Exception {
		...
		if (nsInfo.sendToCC || message.getType() == ChaincodeShim.ChaincodeMessage.Type.KEEPALIVE) {
			if (message.getType() == ChaincodeShim.ChaincodeMessage.Type.KEEPALIVE) {
				...
				logger.info(String.format("[%-8.8s] Send state message %s", message.getTxid(), message.getType()));
			}
			serialSend(message);
		}
	}
