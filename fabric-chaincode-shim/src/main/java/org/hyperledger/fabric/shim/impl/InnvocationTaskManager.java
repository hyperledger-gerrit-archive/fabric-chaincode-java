...
public class InnvocationTaskManager {
	...
	private long submitPauseInterval;
	...
	public InnvocationTaskManager(ChaincodeBase chaincode, ChaincodeID chaincodeId) {
		...
		submitPauseInterval = Long.parseLong((String) props.getOrDefault("TP_SUBMIT_PAUSE_MS","500"));
	...
	public void onChaincodeMessage(ChaincodeMessage chaincodeMessage) {
		logger.fine(
				() -> format("[%-8.8s] %s", chaincodeMessage.getTxid(), ChaincodeBase.toJsonString(chaincodeMessage)));
		...
		case READY:
			...
			break;
		default:
			logger.warning(() -> format("[%-8.8s] Received %s: cannot handle", chaincodeMessage.getTxid(),
	...
	private void sendToTask(ChaincodeMessage message) {
		try {
			...
			logger.error(() -> "Failed to send response to the task task " + message.getTxid() + logger.formatError(e));
	...
	private void newTask(ChaincodeMessage message, Type type) {
		...
		try {
			...
			logger.warn(() -> "Failed to submit task " + message.getTxid() + logger.formatError(e));
			...
			ChaincodeMessage m = ChaincodeMessageFactory.newErrorEventMessage(message.getChannelId(), message.getTxid(), "Failed to submit task for processing");
			this.outgoingMessage.accept(m);
		}
