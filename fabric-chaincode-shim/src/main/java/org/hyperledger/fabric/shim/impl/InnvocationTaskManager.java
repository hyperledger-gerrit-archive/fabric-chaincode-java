...
public class InnvocationTaskManager {
	...
	private void sendToTask(ChaincodeMessage message) {
		try {
			...
			logger.severe(() -> "Failed to send response to the task task " + message.getTxid() + Logger.formatError(e));
	...
	private void newTask(ChaincodeMessage message, Type type) {
		...
		try {
			...
			logger.warning(() -> "Failed to submit task " + message.getTxid() + Logger.formatError(e));
