...
if echo $BINTARGETS | grep -q `uname -m`; then
	...
	if echo $ZSYSTEM_ARCH | grep -q `uname -m`; then
        ...
        cp /root/.m2/repository/io/netty/netty-tcnative-openssl-static/2.0.15.Final/netty-tcnative-openssl-static-2.0.15.Final-linux-s390_64.jar /root/.m2/repository/io/netty/netty-tcnative-openssl-static/2.0.15.Final/netty-tcnative-openssl-static-2.0.15.Final.jar
	else
