...
To write your own chaincode is very easy, although it require undestanding java and gradle.
### Create gradle project
You can use `fabric-chaincode-example-gradle` as staring point. Make sure that your project build creates "uber jar" named chaincode.jar as result.
```
plugins {
...
```
### Writing chaincode
We will use java version of [Simple Asset Chaincode](https://hyperledger-fabric.readthedocs.io/en/latest/chaincode4ade.html#simple-asset-chaincode) as example,
Our chaincode is Go to Java translation of Simple Asset Chaincode, so same explanation works for ut chaincode as well. 


Lets assume our chaincode class is `org.hyperledger.fabric.example.SimpleChaincode`.
First create java file `src/main/java/org/hyperledger/fabric/example/SimpleAsset.java` inside your gradle project.

#### Housekeeping
Your chaincode should implement `Chaincode` interface (or extends `ChaincodeBase` abstract class) and to access `ChaincodeStub` API to access proposal and ledger data.

`ChaincodeBase` class is abstract class inherits `Chaincode` with method `start` used to start chaincode, so we will create our chaincode by extending `ChaincodeBase`
instead implementing `Chaincode`.
...
Chaincode initialization done inside ` Response init(ChaincodeStub stub)` method.
First, retrieve arguments using `ChaincodeStub.getStringArgs()` method.
```java
    /**
...
After that, store state to ledger using using `ChaincodeStub.putStringState(key, value)` method
```java
    /**
     * Init is called during chaincode instantiation to initialize any
     * data. Note that chaincode upgrade also calls this function to reset
...
Chaincode invokation dne inside `Response invoke(ChaincodeStub stub)` method
```java
    /**
...
Extract function name and arguments using `ChaincodeStub.getFunction()` and `ChaincodeStub.getParameters()` methods.
Validate function name and invoke corresponding chaincode methods. Return value returned by chaincode methods as success response payload.
In case of exception or incorrect function value, return error response.
...
Implement methods `set()` and `get()` using `ChaincodeStub.putStringState(key, value)` and `ChaincodeStub.getStringState(key)` 
...
```
Assuming there are no errors, now you can proceed to chaincode testing.

#### Testing chaincode

Install the chaincode. The peer CLI will package the java chaincode source (src folder) and gradle build scripts and send to the peer to install. If you have previously installed a chaincode called by the same name and version, you can delete it from the peer by removing the file /var/hyperledger/production/chaincodes/<name>.<version>.
```
