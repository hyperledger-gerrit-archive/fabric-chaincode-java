...
public class LauncherTest {
	...
	@Test
	public void startWithNoChaincodeId() {
		thrown.expect(Matchers.isA(IllegalArgumentException.class));
		thrown.expectMessage("The chaincode id must be specified using either the -i "
				+ "or --i command line options or the CORE_CHAINCODE_ID_NAME environment variable.");
		when(launcherConfiguration.getId()).thenReturn(null);
		launcher.start(chaincode, new String[]{"arg1", "arg2"});
	}

	@Test
	public void newPeerClientConnectionWithTLSDisabled() {
	...
	private byte[] getX509Certificate() {
